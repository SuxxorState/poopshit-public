...
class NoteSplashDebugState extends MusicBeatState
{
	...
	var skinInputText:FlxInputText;
	...
	override function create()
	{
		...
		var txtx = 60;
		var txty = 100;
		var animName:FlxText = new FlxText(txtx, txty-20, 'Animation name:', 16);
		add(animName);

		nameInputText = new FlxInputText(txtx, txty, 360, '', 16);
		...
		var animName:FlxText = new FlxText(txtx, txty + 50, 'Note Texture File:', 16);
		add(animName);
		
		skinInputText = new FlxInputText(txtx, txty + 70, 360, '', 16);
		skinInputText.callback = function(text:String, action:String)
		{
			switch(action)
			{
				case 'enter':
					skinInputText.hasFocus = false;
					loadFrames();
				default:
					trace('changed splash to $text');
					texturePath = text;
			}
		...
		offsetsText.setFormat("Mars Needs Cunnilingus", 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		offsetsText.scrollFactor.set();
		add(offsetsText);

		curFrameText = new FlxText(300, 100, 680, '', 16);
		curFrameText.setFormat("Mars Needs Cunnilingus", 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		curFrameText.scrollFactor.set();
		add(curFrameText);

		curAnimText = new FlxText(300, 50, 680, '', 16);
		curAnimText.setFormat("Mars Needs Cunnilingus", 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		curAnimText.scrollFactor.set();
		add(curAnimText);

		var text:FlxText = new FlxText(0, 570, FlxG.width,
		...
		text.setFormat("Mars Needs Cunnilingus", 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.scrollFactor.set();
		add(text);

		savedText = new FlxText(0, 340, FlxG.width, '', 24);
		savedText.setFormat("Mars Needs Cunnilingus", 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		var notTyping:Bool = !nameInputText.hasFocus && !skinInputText.hasFocus;
	...
	function loadFrames()
	{
		if (texturePath == '') texturePath = NoteSplash.defaultNoteSplash + NoteSplash.getSplashSkinPostfix();
		splashes.forEachAlive(function(spr:FlxSprite) {
			spr.frames = Paths.getSparrowAtlas('noteSplashes/' + texturePath);
		});
		...
		skinInputText.text = texturePath;
	...
	function saveFile()
	{
		...
		var pathSplit:Array<String> = (Paths.getPath('images/noteSplashes/$texturePath.png', IMAGE, true).split('.png')[0] + '.txt').split(':');
	...
	function changeSelection(change:Int = 0)
	{
		...
		selection.x = curSelected * 220 + 80;
		updateOffsetText();
	}
