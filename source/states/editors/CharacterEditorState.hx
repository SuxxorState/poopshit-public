...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textAnim.setFormat(Paths.font("DTM-Mono.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadBGs() {
		...
		if(!char.isPlayer) playerXDifference = 900;
		...
		/*if(onPixelBG) {
			...
			Paths.setCurrentLevel('poopshitter');
			var bg:BGSprite = new BGSprite('Judgement Hall3', -600 + OFFSET_X - playerXDifference, 0);
			bgLayer.add(bg);
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.dmgOutput = parsedJson.dmg_output;
	...
	var dmgOutputStepper:FlxUINumericStepper;
	...
	function addCharacterUI() {
		...
		dmgOutputStepper = new FlxUINumericStepper(healthIconInputText.x + 120, healthIconInputText.y, 1, 1, 1, 99, 0);
		...
		tab_group.add(new FlxText(dmgOutputStepper.x, dmgOutputStepper.y - 18, 0, 'Damage Output:'));
		...
		tab_group.add(dmgOutputStepper);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == dmgOutputStepper)
			{
				char.dmgOutput = Std.int(dmgOutputStepper.value);//ermm you forgot this??
			}
	...
	function loadChar(isDad:Bool, blahBlahBlah:Bool = true) {
		...
		charLayer.clear();
		ghostChar = new Character(1000, 220, daAnim, !isDad);
		ghostChar.debugMode = true;
		ghostChar.alpha = 0.6;

		char = new Character(1000, 220, daAnim, !isDad);
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			resetHealthBarColor();
			leHealthIcon.changeIcon(healthIconInputText.text, false);
			dmgOutputStepper.value = char.dmgOutput;
