...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Prep Subtitles', "Nothing. Yep, that's right."],
		['Subtitles', "Nothing. Yep, that's right."],

		['FTB', "Fade to Black.\nValue 1: Time in Beats"],
		['FOOB', "Fade out of Black.\nFor BBG's intro and any parts after.\nValue 1: Time in Beats"],
		['BBG BG Black', "Tween the BBG BG gradient.\nValue 1: Time in Beats"],
		['BBG GF Appear', "GF hands.\nValue 1: Time in Beats"],
		['Zoom Camera', "Zoom Camera. Self explanatory.\nValue 1: Zoom (Default: Current Zoom)\nValue 2: Speed Multiplier (Default: Current Speed)"],

		['Uncoverpyrus', "Nothing. Yep, that's right."],

		['Floor Fall', "Nothing. Yep, that's right."],
		['Greyscale Char', "Nothing. Yep, that's right."],
		['Char Fade', "Nothing. Yep, that's right."],
		['Rapping Mode', "Nothing. Yep, that's right."],
		['Switcheroo', "Nothing. Yep, that's right."],

		['Sans', "Nothing. Yep, that's right."],
		['Go Back', "Nothing. Yep, that's right."],
		['Reveal Backup', "Nothing. Yep, that's right."],

		['Papyrus Blaster', "Nothing. Yep, that's right."],
		['Final P Blaster', "Nothing. Yep, that's right."],
	...
	var helpTxt:FlxTypedGroup<FlxText>;
	...
	**/
	...
	var multiInputText:FlxUIInputText;
	...
	override function create()
	{
		...
		if (PlayState.SONG != null) {
			...
			curDifficulty = Difficulty.list[PlayState.storyDifficulty];
		} else {
		...
		bgcolour.antialiasing = ClientPrefs.data.antialiasing;
		add(bgcolour);
		...
		waveformSprite = new FlxSprite(LANE_OFF + (GRID_SIZE * 1.5), 0).makeGraphic(1, 1, 0x00FFFFFF);
		add(waveformSprite);

		var eventIcon:FlxSprite = new FlxSprite(20, 10).loadGraphic(Paths.image('eventArrow'));
		eventIcon.alpha = 0.5;
		...
		leftIcon.setPosition(185, 0);
		rightIcon.setPosition(365, 0);
		...
		bpmTxt = new FlxText(1000, 10, 0, "", 20);
		bpmTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, RIGHT);
		bpmTxt.scrollFactor.set();
		add(bpmTxt);
		secTxt = new FlxText(1000, bpmTxt.y + 25, 0, "", 20);
		secTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, RIGHT);
		secTxt.scrollFactor.set();
		add(secTxt);
		beatTxt = new FlxText(1000, secTxt.y + 25, 0, "", 20);
		beatTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, RIGHT);
		beatTxt.scrollFactor.set();
		add(beatTxt);
		stepTxt = new FlxText(1000, beatTxt.y + 25, 0, "", 20);
		stepTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, RIGHT);
		stepTxt.scrollFactor.set();
		add(stepTxt);

		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE * 10) + 20, 4);
		strumLine.offset.x = 10;
		add(strumLine);
		...
		UI_box = new FlxUITabMenu(null,	[{name: "Song", label: 'Song'}, {name: "Section", label: 'Section'}, {name: "Data", label: 'Data'}, {name: "Charting", label: 'Charting'}], true);
		UI_box.resize(350, 400);
		UI_box.x = Std.int(FlxG.width * 0.6) + GRID_SIZE / 2;
		UI_box.y = 310;
		UI_box.scrollFactor.set();

		NOTE_box = new FlxUITabMenu(null, [{name: "Note", label: 'Note'}, {name: "Events", label: 'Events'}], true);
		NOTE_box.resize(300, 160);
		NOTE_box.x = Std.int(FlxG.width * 0.6) + 25 + GRID_SIZE / 2;
		NOTE_box.y = 130;
		NOTE_box.scrollFactor.set();
		...
		helpTxt = new FlxTypedGroup<FlxText>();
		var tipTextArray:Array<String> = text.split('\n');
		for (i in 0...tipTextArray.length) {
			var tipText:FlxText = new FlxText(20, 50, 0, tipTextArray[i], 16);
			tipText.y += i * 12;
			tipText.x -= 500;
			tipText.setFormat(Paths.font("Mars_Needs_Cunnilingus.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var helpButton:FlxButton = new FlxButton(20, FlxG.height - 7, "Keybinds", function(){showKeybinds();});
		...
		quantTxt = new FlxText(10, FlxG.height - 20, 0, "Beat Snap: " + quantization + "th", 20);
		quantTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, LEFT);
		quantTxt.updateHitbox();
		quantTxt.y -= quantTxt.height + 10;
		quantTxt.scrollFactor.set();
		add(quantTxt);

		zoomTxt = new FlxText(10, FlxG.height - 10, 0, "Zoom: 1 / 1", 20);
		zoomTxt.setFormat("Mars Needs Cunnilingus", 20, FlxColor.WHITE, LEFT);
	...
	function showKeybinds() {
		if (!keybindsShown) {
			keybindBGtween = FlxTween.tween(keybindBG, {x: 0, alpha: 1}, 1, {ease: FlxEase.elasticOut, onComplete:
			...
			for (help in helpTxt) {
				var cool = help.ID;
				keybindTweens.set('keybind $cool', FlxTween.tween(help, {alpha: 1, x: 20}, 1, {ease: FlxEase.elasticOut}));
			}
		} else {
			keybindBGtween = FlxTween.tween(keybindBG, {x: -500, alpha: 0}, 1, {ease: FlxEase.elasticIn, onComplete:
				function (twn:FlxTween) {
					keybindBGtween = null;
				}
			});
			for (help in helpTxt.members) {
				var cool = help.ID;
				keybindTweens.set('keybind $cool', FlxTween.tween(help, {alpha: 0, x: -520}, 1, {ease: FlxEase.elasticIn}));
			}
		}
	...
	function addSongUI():Void
	{
		...
		var stepperSpeed:FlxUINumericStepper = new FlxUINumericStepper(stepperBPM.x + 70, stepperBPM.y, 0.1, 1, 0.1, 10, 2);
		...
		var player1DropDown = new FlxUIDropDownMenu(10, stepperSpeed.y + 100, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var gfVersionDropDown = new FlxUIDropDownMenu(player1DropDown.x + 140, player1DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var player2DropDown = new FlxUIDropDownMenu(player1DropDown.x, gfVersionDropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var difficulties:Array<String> = Difficulty.defaultList;
		...
		var difficultyDropDown = new FlxUIDropDownMenu(10, stepperSpeed.y + 35, FlxUIDropDownMenu.makeStrIdLabelArray(difficulties, true), function(character:String)
		{
			curDifficulty = Difficulty.list[Std.parseInt(character)];
		...
		if (difficulties.length > 1) tab_group_song.add(new FlxText(difficultyDropDown.x, difficultyDropDown.y - 15, 0, 'Difficulty:'));
		...
		if (difficulties.length > 1) tab_group_song.add(difficultyDropDown);
	...
	function addSectionUI():Void
	{
		...
		stepperSectionBPM = new FlxUINumericStepper(check_duetSection.x, check_changeBPM.y + 20, 1, Conductor.bpm, 0, 999, 1);
		...
		sectionInputText = new FlxUIInputText(130, 113, 80, "");
		...
		multiInputText = new FlxUIInputText(150, 153, 60, "");
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'Section Beats:'));
		...
		tab_group_section.add(new FlxText(check_copyMulti.x, multiInputText.y - 15, 0, 'Copy Multiple Sections'));
	...
	var leftClickDropDown:FlxUIDropDownMenu;
	var rightClickDropDown:FlxUIDropDownMenu;
	...
	{
		var tab_group_note = new FlxUI(null, NOTE_box);
		...
		strumTimeInputText = new FlxUIInputText(stepperSusLength.x + 140, stepperSusLength.y, 120, "0");
		...
		leftClickDropDown = new FlxUIDropDownMenu(10, 65, FlxUIDropDownMenu.makeStrIdLabelArray(clickNameList, true), function(character:String)
		{
			leftClickType = Std.parseInt(character);
		});
		leftClickDropDown.selectedLabel = clickNameList[leftClickType];

		rightClickDropDown = new FlxUIDropDownMenu(150, 65, FlxUIDropDownMenu.makeStrIdLabelArray(clickNameList, true), function(character:String)
		{
		...
		tab_group_note.add(new FlxText(150, 10, 0, 'Strum time (in MS):'));
		tab_group_note.add(new FlxText(10, 50, 0, "Left Click:"));
		tab_group_note.add(new FlxText(150, 50, 0, "Right Click:"));
		tab_group_note.add(new FlxText(10, 90, 0, "Selected Note's type:"));
		...
		NOTE_box.addGroup(tab_group_note);
	}
	...
	{
		var tab_group_event = new FlxUI(null, NOTE_box);
		...
		var text:FlxText = new FlxText(20, 10, 0, "Event:");
		tab_group_event.add(text);
		eventDropDown = new FlxUIDropDownMenu(20, 25, FlxUIDropDownMenu.makeStrIdLabelArray(leEvents, true), function(pressed:String) {
		...
		var text:FlxText = new FlxText(20, 70, 0, "Value 1:");
		tab_group_event.add(text);
		value1InputText = new FlxUIInputText(20, 85, 100, "");
		blockPressWhileTypingOn.push(value1InputText);

		var text:FlxText = new FlxText(150, 70, 0, "Value 2:");
		tab_group_event.add(text);
		value2InputText = new FlxUIInputText(150, 85, 100, "");
		...
		var copyEventButton:FlxButton = new FlxButton(20, 110, 'Copy', function()
		{
			if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
			{
				var eventsGroup:Array<Dynamic> = curSelectedNote[1];
		...
		var PANEButton:FlxButton = new FlxButton(copyEventButton.x + 100, 110, 'Paste', function()
		{
			if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
			{
				var eventsGroup:Array<Dynamic> = curSelectedNote[1];
		...
		NOTE_box.addGroup(tab_group_event);
	}
	...
	function addChartingUI() {
		...
		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_ovocals.y + 30, null, null, 'Play Sound (Boyfriend notes)', 100,
			function() {
	...
	function addDataUI()
	{
		...
		var gameOverTypeDropDown = new FlxUIDropDownMenu(180, 24, FlxUIDropDownMenu.makeStrIdLabelArray(GameOverSubstate.gameOverTypes, true), function(type:String) {
			_song.gameOverType = GameOverSubstate.gameOverTypes[Std.parseInt(type)];
		});
		gameOverTypeDropDown.selectedLabel = _song.gameOverType;
		blockPressWhileScrolling.push(gameOverTypeDropDown);
		...
		tab_group_data.add(gameOverTypeDropDown);
		...
		tab_group_data.add(new FlxText(gameOverTypeDropDown.x, gameOverTypeDropDown.y - 15, 0, 'Game Over Type:'));
	...
	function loadSong():Void
	{
		...
		var pfile:Dynamic = Paths.pVoices(currentSongName);
		var ofile:Dynamic = Paths.oVoices(currentSongName);
		pvocals = new FlxSound();
		if (Std.isOfType(pfile, Sound) || OpenFlAssets.exists(pfile)) {
			pvocals.loadEmbedded(pfile);
			pvocals.autoDestroy = false;
	...
	override function closeSubState()
	{
		if(playtesting)
		{
			...
			{
				pvocals.pause();
				pvocals.time = playtestingTime;
				if (pVoicesVolume != null) pvocals.volume = pVoicesVolume.value;
			...
			{
				ovocals.pause();
				ovocals.time = playtestingTime;
				if (oVoicesVolume != null) ovocals.volume = oVoicesVolume.value;
	...
	override function update(elapsed:Float)
	{
		...
		if ((leftClickType != curNoteTypes.length + 1 && FlxG.mouse.justPressed) || (rightClickType != curNoteTypes.length + 1 && FlxG.mouse.justPressedRight))
		{
	...
	*/
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			var width:Int = Std.int(GRID_SIZE * 8.5);
			var height:Int = Std.int(eventGrid.height);
		...
		var hSize:Int = Std.int(gSize / 2.125);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("Mars_Needs_Cunnilingus.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -27;
				daText.yAdd = 10;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("Mars_Needs_Cunnilingus.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	private function addNote(rightClick:Bool = false, strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if (leftClickType < curNoteTypes.length && !rightClick)
			daType = leftClickType;
		else if (rightClickType < curNoteTypes.length && rightClick)
	...
	function loadJson(song:String):Void
	{
		...
		try {
			if (curDifficulty != Difficulty.getDefault()) {
		...
		catch(e)
		{
			...
			if(missingText == null)
			{
				...
				missingText.setFormat(Paths.font("Mars_Needs_Cunnilingus.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	private function saveLevel()
	{
		...
		if (Difficulty.getDefault() != curDifficulty)
